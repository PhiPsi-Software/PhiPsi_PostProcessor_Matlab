function h = Tools_mArrow3(p1,p2,varargin)
%https://www.mathworks.com/matlabcentral/fileexchange/25372-marrow3-m-easy-to-use-3d-arrow

%Tools_mArrow3 - plot a 3D arrow as patch object (cylinder+cone)
%
% syntax:   h = Tools_mArrow3(p1,p2)
%           h = Tools_mArrow3(p1,p2,'propertyName',propertyValue,...)
%
...
%           'facealpha'.)
%                       
% example1: h = Tools_mArrow3([0 0 0],[1 1 1])
%           (Draws an arrow from [0 0 0] to [1 1 1] with default properties.)
%
% example2: h = Tools_mArrow3([0 0 0],[1 1 1],'color','red','stemWidth',0.02,'facealpha',0.5)
    ...
    switch varargin{argno}
        ...
        case 'stemWidth'
            if isreal(varargin{argno+1})
                ...
                warning('Tools_mArrow3:stemWidth','stemWidth must be a real number');
            end
        case 'tipWidth'
            if isreal(varargin{argno+1})
                tipWidth = varargin{argno+1};
            else
                warning('Tools_mArrow3:tipWidth','tipWidth must be a real number');
            end
        otherwise
