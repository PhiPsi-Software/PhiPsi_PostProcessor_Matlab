...
function Animate_Main_3D(Real_num_iteration)
...
% if Key_Animation(1)==1 | Key_Animation(1)==2
    % Animate_Node_and_Gauss_Disp_3D(Real_num_iteration)
% end

% Generate animations of nodal stress contours
% if Key_Animation(2)==1 | Key_Animation(2)==2
    % Animate_Node_and_Gauss_Stress_3D(Real_num_iteration)
% end

% Generate deformation animations
if Key_Animation(3)==1
    Animate_Deformation_3D(Real_num_iteration)
end

% 场变量动画绘制
% if Key_Animation(4)~=0
    % Animate_Fd_Value_3D(Real_num_iteration)
% end
